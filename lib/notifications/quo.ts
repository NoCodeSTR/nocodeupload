 ...
 * with a rule's messageTemplate we render it (tokens: {name}, {message},
 * {link}, {field:Label}, {count}, …) so owners can write their own copy
...
export async function sendQuoForUpload(
  ...
  if (message && message.trim()) {
    ...
    content = `New upload to ${name}: ${u.original_filename} (${fileCategory(u.mime_type)})${by}.` + (url ? ` ${url}` : "");
  }
...
export async function sendQuoForBatch(
  ...
  if (message && message.trim()) {
    ...
    content = `${uploads.length} files uploaded to ${name}${by}.` + (firstUrl ? ` First: ${firstUrl}` : "");
  }
